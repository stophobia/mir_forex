    ...
    <div class="card">
        <div class="card-header">
            <div class="d-flex">
                ...
                <p class="mb-0 d-inline-block ms-auto me-0 fw-bolder text-uppercase text-muted">{{code}}</p>
            </div>
            <p class="mb-0 text-muted ms-0 me-0">{{date}}</p>
        </div>
        <div class="card-body" v-bind:class="{ 'long-text': long }">
            <button class="down" data-bs-toggle="modal" data-bs-target="#reviewModal" style="z-index: 99;" v-on:click="openWindow()">
                <font-awesome-icon :icon="['fas', 'chevron-down']" size="xl" style="color: rgba(255, 255, 255, .3);"/>
            </button>
            <p>{{text}}</p>
        </div>
    </div>
